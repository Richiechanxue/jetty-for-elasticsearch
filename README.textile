...
<pre>
Download the desired version from https://github.com/yuderbin/jetty-for-elasticsearch/releases and unzip it to plugins/jetty-1.7.2.
</pre>
...
<pre>
sonian.elasticsearch.http.type: com.sonian.elasticsearch.http.jetty.JettyHttpServerModule
</pre>

The elasticsearch-jetty plugin adds @Server: Jetty(8.1.17.v20150415)@ header to all responses. So, it's possible to verify that jetty plugin is running by checking the response headers using the following curl command:
...
<pre>
http.type: com.sonian.elasticsearch.http.filter.FilterHttpServerModule
</pre>
